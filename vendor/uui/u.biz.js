...
//		this.comps = {}
	...
	App.fn.init = function(viewModel, element){
		...
		$(this.element).find('[data-u-meta]').each(function() {
			if ($(this).data('data-u-meta')) return
			if ($(this).parents('[data-u-meta]').length > 0) return 
			var options = JSON.parse($(this).attr('data-u-meta'))
			if(options && options['type']) {
				if (self.adjustFunc)
...
//				self._successFunc(data, deferred)
	...
	ServerEvent.fn.getData = function(){
		return 	{
			environment: ko.toJSON(this.app.getEnvironment()),
			dataTables: ko.toJSON(this.datas)
		}
	}
